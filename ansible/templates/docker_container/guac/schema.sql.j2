...
SELECT
    entity_id,
    UNHEX(SHA2(CONCAT('{{ domain_admin_password }}', HEX(@salt)), 256)),  -- ''
    @salt,
    NOW()
...
INSERT INTO guacamole_entity (name, type) VALUES ('{{ guac_ldap_admin_user }}', 'USER');
...
SELECT
    ...
    @salt,
    NOW()
FROM guacamole_entity WHERE name = '{{ guac_ldap_admin_user }}';

-- Grant this user all system permissions
INSERT INTO guacamole_system_permission (entity_id, permission)
SELECT entity_id, permission
FROM (
          SELECT '{{ guac_ldap_admin_user }}'  AS username, 'CREATE_CONNECTION'       AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}'  AS username, 'CREATE_CONNECTION_GROUP' AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}'  AS username, 'CREATE_SHARING_PROFILE'  AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}'  AS username, 'CREATE_USER'             AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}'  AS username, 'CREATE_USER_GROUP'       AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}'  AS username, 'ADMINISTER'              AS permission
) permissions
JOIN guacamole_entity ON permissions.username = guacamole_entity.name AND guacamole_entity.type = 'USER';

-- Create user group
INSERT INTO guacamole_entity (name, type) VALUES ('{{ guac_ldap_admin_group }}', 'USER_GROUP');

INSERT INTO guacamole_user_group (entity_id)
SELECT
    entity_id
FROM guacamole_entity WHERE name = '{{ guac_ldap_admin_group }}';

-- Grant this group all system permissions
INSERT INTO guacamole_system_permission (entity_id, permission)
SELECT entity_id, permission
FROM (
          SELECT '{{ guac_ldap_admin_group }}'  AS groupname, 'CREATE_CONNECTION'       AS permission
    UNION SELECT '{{ guac_ldap_admin_group }}'  AS groupname, 'CREATE_CONNECTION_GROUP' AS permission
    UNION SELECT '{{ guac_ldap_admin_group }}'  AS groupname, 'CREATE_SHARING_PROFILE'  AS permission
    UNION SELECT '{{ guac_ldap_admin_group }}'  AS groupname, 'CREATE_USER'             AS permission
    UNION SELECT '{{ guac_ldap_admin_group }}'  AS groupname, 'CREATE_USER_GROUP'       AS permission
    UNION SELECT '{{ guac_ldap_admin_group }}'  AS groupname, 'ADMINISTER'              AS permission
) permissions
...
SELECT guacamole_entity.entity_id, guacamole_user.user_id, permission
FROM (
          SELECT 'guacadmin' AS username, '{{ guac_ldap_admin_user }}' AS affected_username, 'READ'       AS permission
    UNION SELECT 'guacadmin' AS username, '{{ guac_ldap_admin_user }}' AS affected_username, 'UPDATE'     AS permission
    UNION SELECT 'guacadmin' AS username, '{{ guac_ldap_admin_user }}' AS affected_username, 'ADMINISTER' AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}' AS username, '{{ guac_ldap_admin_user }}' AS affected_username, 'READ'     AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}' AS username, '{{ guac_ldap_admin_user }}' AS affected_username, 'UPDATE' AS permission
) permissions
JOIN guacamole_entity          ON permissions.username = guacamole_entity.name AND guacamole_entity.type = 'USER'
JOIN guacamole_entity affected ON permissions.affected_username = affected.name AND guacamole_entity.type = 'USER'
JOIN guacamole_user            ON guacamole_user.entity_id = affected.entity_id;
...
-- Grant admin permission to read/update/administer self
INSERT INTO guacamole_user_group_permission (entity_id, affected_user_group_id, permission)
SELECT guacamole_entity.entity_id, guacamole_user_group.user_group_id, permission
FROM (
          SELECT '{{ guac_ldap_admin_user }}' AS username, '{{ guac_ldap_admin_group }}' AS affected_groupname, 'READ'       AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}' AS username, '{{ guac_ldap_admin_group }}' AS affected_groupname, 'UPDATE'     AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}' AS username, '{{ guac_ldap_admin_group }}' AS affected_groupname, 'DELETE'     AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}' AS username, '{{ guac_ldap_admin_group }}' AS affected_groupname, 'ADMINISTER' AS permission
) permissions
JOIN guacamole_entity          ON permissions.username = guacamole_entity.name AND guacamole_entity.type = 'USER'
JOIN guacamole_entity affected ON permissions.affected_groupname = affected.name AND guacamole_entity.type = 'USER_GROUP'
JOIN guacamole_user_group      ON guacamole_user_group.entity_id = affected.entity_id;

-- Create Connection Group
INSERT INTO guacamole_connection_group (connection_group_name, type) VALUES ('{{ guac_root_connection_group }}','ORGANIZATIONAL');
...
FROM (
          SELECT '{{ guac_ldap_admin_user }}' AS username, '{{ guac_root_connection_group }}' AS affected_connection_group_name, 'READ'        AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}' AS username, '{{ guac_root_connection_group }}' AS affected_connection_group_name, 'UPDATE'      AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}' AS username, '{{ guac_root_connection_group }}' AS affected_connection_group_name, 'DELETE'      AS permission
    UNION SELECT '{{ guac_ldap_admin_user }}' AS username, '{{ guac_root_connection_group }}' AS affected_connection_group_name, 'ADMINISTER'  AS permission
...
SELECT guacamole_entity.entity_id, guacamole_connection_group.connection_group_id, permission
FROM (
          SELECT '{{ guac_ldap_admin_group }}' AS groupname, '{{ guac_root_connection_group }}' AS affected_connection_group_name, 'READ' AS permission
