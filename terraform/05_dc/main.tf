...
module "azure_vms" {
  ...
  name                        = "vm-${var.environment_prefix}-dc"
  hostname                    = "dc"
  instance_count              = 1
  size                        = "Standard_B1ms"
  ...
  vm_os_sku       = "2022-datacenter-core-smalldisk"
  ...
  content = templatefile("../_files/ansible_inventory.tmpl",
    {
      ...
      ansible_group      = substr(basename(path.cwd), -2, -1) # dc
      is_windows_image   = true
      ansible_host_vars  = ""
      ansible_hostgroups = ["windows_ca"]
    }
  )
